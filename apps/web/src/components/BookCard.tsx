...
import { subcategoryLabel, verticalAccent, verticalById } from "@/lib/verticals";
  ...
  return (
    <Link href={`/book/${book.slug}`} className={`book-card${compact ? " compact" : ""}`} aria-label={`Open ${book.title}`}>
      ...
      <span className="by" style={{ color: accent }}>
        {subcategoryLabel(book.vertical, book.subcategory) ?? verticalById(book.vertical)?.label ?? book.vertical}
        {compact ? "" : ` · ${book.author}`}
      </span>
      <h3>{book.title}</h3>
